...
const {
  ...
  ModalBuilder, TextInputBuilder, TextInputStyle,
  REST, Routes, SlashCommandBuilder
>>>>>>> 9f3db9e716a3de42a43f8db75d3bda9353c0ecf6
} = require('discord.js');
...
function drawGraph(data, crashed = false) {
    ...
    const maxY = Math.max(currentVal * 1.15, 2.0);
    ...
    const totalPoints = Math.max(data.length, 30);
    ...
    if (data.length > 0) {
        ...
        ctx.arc(lastX,
            lastY, 12, 0, Math.PI * 2);
        ctx.fillStyle = mainColor.replace('1)', '0.3)');
        ctx.fill();
    }
